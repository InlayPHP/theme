...
describe('theme helpers', () => {
  it('provides base and Orbit default presets', () => { expect(baseTheme.tokens.radius).toBe('0.5rem'); expect(baseTheme.tokens['button-height']).toBe('2.5rem'); expect(baseTheme.tokens['button-xs-height']).toBe('2rem'); expect(baseTheme.tokens['control-border']).toBe('#d4d4d8'); expect(baseTheme.tokens['space-card']).toBe('1.25rem'); expect(baseTheme.tokens['font-size-body']).toBe('0.875rem'); expect(baseTheme.tokens['focus-ring-color']).toBe('var(--inlay-accent)'); expect(baseTheme.tokens['motion-duration']).toBe('160ms'); expect(orbitTheme.name).toBe('orbit'); expect(orbitTheme.tokens.accent).toBe('#5b64db'); expect(defaultTheme.tokens.accent).toBe('#5b64db'); expect(defaultTheme.tokens['control-height']).toBe('2.75rem'); expect(defaultTheme.tokens['sidebar-width']).toBe('15.5rem'); expect(defaultTheme.tokens['topbar-height']).toBe('4.5rem'); expect(defaultTheme.tokens['sidebar-active']).toBe('#e4eaff') })
